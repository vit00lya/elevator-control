...
Работает в связке с лифтами, воротами, дверьми.
...
При начале работы устройство блокируется для открытия.

Оператор которому нужно открыть устройство последовательно считывает штрихкоды продукции которые необходимо отгрузить с склада.
На дисплее после каждого считывания высвечивается название позиции.
При считывании служебного штрихкода 9999999999999 устройство разблокируется на 10 минут и данные передаются на HTTP-сервер.
При считывании служебного штрихкода 9999999999998 устройство разблокируется на 1 минуту, затем закрывается автоматически.
...
Переферия сканер штрихкодов, 2-х строчный индикатор, реле для нажатия на кнопки устройства.

Для сборки используется CMAKE.

При сборке, если используется дисплей и управление устройствами необходимо использовать переменную EXTERNAL_DISPLAY.
